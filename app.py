...
from pinecone_utils import initialize_pinecone_rag_index, get_user_embeddings, delete_embeddings, get_user_rag_stats
...
def main_page():
    ...
    # Custom CSS for sidebar buttons to make them same width and color logout button
    st.markdown(
        """
        <style>
        ...
        div[data-testid="stSidebarNav"] button:last-child { /* Target the Logout button */
            background-color: #FF4B4B; /* Red color for danger */
            color: white;
        }
        div[data-testid="stSidebarNav"] button:last-child:hover {
    ...
    # Sidebar buttons
    with st.sidebar:
        if st.button("Home", key="sidebar_home_btn", type="primary"):
            set_page("main")
        if st.button("Admin Page", key="sidebar_admin_btn", type="primary"):
            set_page("admin")
        if st.button("Logout", key="sidebar_logout_btn", type="secondary"): # Use secondary type for custom styling
    ...
    # Display statistics
    st.subheader("Your RAG Statistics")
    user_id = st.session_state["user_id"]
    if user_id:
        stats = get_user_rag_stats(rag_index, user_id)
        st.info(f"Total documents inserted: {stats['total_documents']}")
        st.info(f"Total chunks stored: {stats['total_chunks']}")
    else:
        st.info("Login to see your RAG statistics.")
...
def admin_page():
    ...
    # Sidebar buttons
    with st.sidebar:
        if st.button("Home", key="admin_sidebar_home_btn", type="primary"):
            set_page("main")
        if st.button("Admin Page", key="admin_sidebar_admin_btn", type="primary"):
            set_page("admin")
        if st.button("Logout", key="admin_sidebar_logout_btn", type="secondary"): # Use secondary type for custom styling
            st.session_state["logged_in"] = False
            st.session_state["username"] = None
            st.session_state["user_id"] = None
            st.session_state["page"] = "main" # Reset page on logout
